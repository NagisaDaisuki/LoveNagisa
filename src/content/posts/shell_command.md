...
description: "Linux 是什么!?"
category: Linux
tags: ["linux", "shell", "command", "fucntion", "c"]
...
1. **与使用者或用户组有关的参数：`-uid n`,`-gid n`,`-user name`,`-group name`,`-nouser`,`-nogroup`**
...
| --------------- | ------------------------------------------ |
| 按名查          | `-name` / `-iname`                         |
| 按类型          | `-type f` / `-type d` / `-type l`          |
| 按时间          | `-mtime -n` / `-newer`                     |
| 按大小          | `-size +100M` / `-size -1k` / `-size 100c` |
| 空文件          | `-empty`                                   |
| 权限/可执行文件 | `-perm 755` / `-executable` / `-perm -111` |
| 排除条件        | `! -name`                                  |
| 删除            | `-delete`                                  |
| 执行命令        | `-exec` 或配合 `xargs`                     |
| 限制深度        | `-maxdepth 1`、`-mindepth 2`               |
| 跳过目录        | `-prune`                                   |
| 组合条件        | `-a` / `-o` / `!`                          |
...
1. 创建 Linux/Windows 兼容的 ISO 镜像 (常用)：
...
1. **制作可引导 USB 驱动器 (从 ISO 镜像)：**

```bash
sudo dd if=ubuntu-24.04-desktop-amd64.iso of=/dev/sdX bs=4M status=progress
```

1. **创建大文件（例如用于交换文件或测试文件）：**
